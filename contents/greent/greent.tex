...
Il va d'abord organiser les différents tests en un concept que nous avons appelé \textit{Bundle}, ceci dans un but d'optimisation du temps d'exécution. En effet, si nous avons 1000 tests de 2 minutes, cela ferait plus de trente heures d'exécution. Pour palier à ce problèmes, deux stratégies :
\begin{itemize}
	\item Nous regroupons les même scénarios ensembles afin de n'exécuter qu'une seule fois le scénario, et d'évaluer tous les tests correspondant sur les même traces. Cela évite d'exécuter plusieurs fois les même scénarios, tel qu'une rampe de vitesse. Ce regroupement est appelé << Bundle >>.
	\item Même grâce à notre stratégie des Bundles, l'exécution pourrait être encore trop longue. Ainsi, il est possible d'utiliser plusieurs tables en simultanées, afin de diminuer le temps d'exécution par le nombre de tables.
\end{itemize}
...
\section{Le fonctionnalités des serveurs}
